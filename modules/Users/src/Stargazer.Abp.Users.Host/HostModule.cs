...
using Stargazer.Abp.Users.EntityFrameworkCore;
using Stargazer.Abp.Users.HttpApi;
...
namespace Stargazer.Abp.Users.Host
{
    [DependsOn(
    ...
    public class HostModule : AbpModule
    {
        ...
        private void ConfigureDataProtection(ServiceConfigurationContext context, IConfiguration configuration)
        {
            ...
            var dataProtectionBuilder = context.Services.AddDataProtection()
                .SetApplicationName(Assembly.GetExecutingAssembly().FullName ?? "Stargazer.Abp.EShop");
            if (configuration["Redis:IsEnabled"].ToBool())
            {
        ...
        public override void PreConfigureServices(ServiceConfigurationContext context)
        {
            ...
            {
                options.ConventionalControllers.Create(typeof(ApplicationModule).Assembly, ops =>
                {
                    ops.RootPath = "stargazer";
                });
            });
        }
